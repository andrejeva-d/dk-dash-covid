...
raw_data <- read_delim(
  ...
  escape_double = FALSE,
  col_types = cols(
    Antal_testede = col_number(),
    `Antal_bekr\303\246ftede_COVID-19` = col_number(),
    Befolkningstal = col_number(),
    `Kumulativ_incidens_(per_100000)` = col_number()
  ),
  trim_ws = TRUE
)
raw_data <- raw_data[, -1]
colnames(raw_data) <-
  c("Kommune",
    ...
    "Population",
    "Positive_100000")
raw_data <- data.frame(lapply(raw_data, function(x) {
  gsub("\\.", "", x)
}))
raw_data[, c(2, 3, 4, 5)] <-
  data.frame(lapply(raw_data[, c(2, 3, 4, 5)], as.numeric))
...
githubURL <-
  "https://github.com/andrejeva-d/dk-dash-covid/blob/main/Data/geo_data_municipality.RData?raw=true"
load(url(githubURL))
...
REZ_DONE <- merge(REZ,
                  ...
                  by.y = "Kommune",
                  all = TRUE)
save(REZ_DONE, geoj, file = "Data/plot_dk_map_municip.RData")
